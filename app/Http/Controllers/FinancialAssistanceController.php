...
class FinancialAssistanceController extends Controller
{
    ...
    /**
     * Получить сохраненный PDF или сгенерировать новый
     */
    public function getOrGenerateRequestPdf(FinancialAssistanceRequest $request)
    {
        // Проверяем, есть ли уже сохраненный PDF
        if ($request->pdf_path && Storage::disk('public')->exists($request->pdf_path)) {
            $filePath = Storage::disk('public')->path($request->pdf_path);
            $fileName = 'financial_assistance_request_' . $request->id . '.pdf';
            
            return response()->download($filePath, $fileName);
        }

        // Если PDF нет, генерируем новый и сохраняем
        $pdfService = new \App\Services\FinancialAssistancePdfService();
        $savedPath = $pdfService->generateAndSavePdf($request);
        
        if ($savedPath && Storage::disk('public')->exists($savedPath)) {
            $filePath = Storage::disk('public')->path($savedPath);
            $fileName = 'financial_assistance_request_' . $request->id . '.pdf';
            
            return response()->download($filePath, $fileName);
        }

        // Если не удалось сохранить, генерируем временный PDF
        return $this->generateRequestPdf($request);
    }
    ...
    public function generateRequestPdf(FinancialAssistanceRequest $request)
    {
        ...
        $processedHtml = $this->pdfService->generateFullHtmlDocument(
            $request,
            $request->sotrudnik,
            $request->signer
        );
...
//        $pdf->Ln(5);
    ...
    private function addPdfToPdf(Fpdi $pdf, $file): void
    {
        ...
        try {
            ...
            \Log::warning('PDF compression not supported in controller', [
            ...
            \Log::error('PDF parser error in controller', [
                'file_name' => $file->original_name,
                'file_path' => $pdfPath,
                'error_message' => $e->getMessage()
            ]);
            ...
            \Log::error('PDF merge error in controller', [
                'file_name' => $file->original_name,
                'file_path' => $pdfPath,
