...
//                }
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function detail(DetailPayrollSlipRequest $request)
    {
        // Получение аутентифицированного сотрудника
        $sotrudnik = auth()->user();
        $id = $request->input('id');
...
//            $pdfPath = Storage::path($payrollSlip->pdf_path);
//            $imagePath = Storage::path('payroll_slips/images/' . $payrollSlip->id . '.png');
//
//            // Использование команды `convert` для преобразования PDF в изображение
//            exec("convert -density 150 {$pdfPath}[0] -quality 90 {$imagePath}", $output, $return_var);
//
//            if ($return_var !== 0) {
//                throw new \Exception('Ошибка при конвертации PDF в изображение.');
//            }
//
...
//
//            // Опционально: Удалите временное изображение после конвертации
//            unlink($imagePath);
//        } catch (\Exception $e) {
//            Log::error('Ошибка при конвертации PDF в изображение: ' . $e->getMessage());
//
//            return response()->json([
//                'success' => false,
...
//        }
        ...
        return response()->json([
            ...
            'data' => [
                'id' => $payrollSlip->id,
                'last_name' => $payrollSlip->last_name,
                'first_name' => $payrollSlip->first_name,
                'father_name' => $payrollSlip->father_name,
                'tabel_nomer' => $payrollSlip->tabel_nomer,
                'month' => $payrollSlip->month,
//                'pdf_image' => $base64Image, // Возвращаем изображение в base64
                'created_at' => $payrollSlip->created_at,
            ],
        ]);
    }
