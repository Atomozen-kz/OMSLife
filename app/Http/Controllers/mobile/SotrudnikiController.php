...
//        }
    ...
    public function birthdaysList(Request $request)
    {
        ...
        $lang == 'ru' ?
            $text = 'Дорогие коллеги!
Поздравляем наших именинников! Желаем крепкого здоровья, новых свершений, счастья и неиссякаемой энергии. Пусть все задуманное сбывается, а жизнь дарит только приятные сюрпризы!
С теплом, команда ТОО ОзенМунайСервис'
            : $text = 'Қымбатты әріптестер!
Мерейтой иелерін құттықтаймыз! Зор денсаулық, жаңа табыстар, бақыт және шексіз энергия тілейміз.Барлық тілектеріңіз орындалып, өмір жақсы сәттерге толы болсын!
Жылы лебізбен, "ОзенМунайСервис" ЖШС ұжымы';
        try {
            $today = now();
